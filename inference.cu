...
__global__ void conv2d_kernel(const float* input, const float* weights, const float* bias, float* output, int input_size, int kernel_size, int output_size);
__global__ void ifnode_kernel(const float* input, float* membrane_potential, float* output, int size);
__global__ void maxpool2d_kernel(const float* input, float* output, int input_size, int pool_size, int stride);
__global__ void linear_kernel(const float* input, const float* weights, const float* bias, float* output, int input_size, int output_size);
...
__global__ void conv2d_kernel(
    ...
    const float* weights,  // 卷积核权重
    const float* bias,     // 卷积偏置
    float* output,         // 卷积层输出
    int input_size,        // 输入尺寸
...
__global__ void ifnode_kernel(
    const float* input,  // 输入信号（可能是卷积层或全连接层的输出）
    float* membrane_potential, // 膜电位
    float* output,           // 输出脉冲（0 或 1）
    int size                 // 输入的元素个数
)
{
